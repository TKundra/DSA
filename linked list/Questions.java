...
class Questions {
    ...
    static void reverseWithoutRecursion(Node head) {
        Node node = head;
    ...
    static Node mergeKSortedLL(Node[] list) {
        ...
        pq.addAll(Arrays.asList(list).subList(0, list.length));
        ...
        while (!pq.isEmpty()) {
            ...
            // take next node from same list and insert it into min-heap
            if (min.next != null)
                pq.add(min.next);
        }
        return head;
