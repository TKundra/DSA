...
public class RottenOranges {
    ...
    static boolean isValid(int i, int j, int[][] array) {
        return (i >= 0 && j >= 0 && i < r && j < c && array[i][j] == 1);
    }
    ...
    */
    static int rotOranges(int[][] array) {
        ...
        // store all cells having rotten oranges and fresh oranges in first time frame to have a total number of oranges
        for (int i = 0; i < r; i++) {
            for (int j = 0; j < c; j++) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // check right
                if (isValid(pair.x+1, pair.y, array)) {
                    // make oranges rotten
                ...
                // check top
                if (isValid(pair.x, pair.y + 1, array)) {
                    // make oranges rotten
        ...
        O(R*C)
        each element of matrix be inserted into queue so upper bound of iteration is O(R*C)
    */
